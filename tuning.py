...
def tune_hyperparameter(X_tr, y_tr_01):
    ...
    gamma_samples = [
        1
    ]  # put to 1 because adaptif by shcedule during training each epoch
...
def save_tuning_results(results_list, filepath_csv=config.TUNING_PATH):
    ...
    with open(filepath_csv, "w") as f:
        ...
        for r in results_list:
            f.write(
                f"{r['lambda']:.9f},{r['gamma']:.6f},{r['max_iters']},"
    ...
    print(
        f"[Saved] All grid search results ({len(results_list)} combinations) -> {filepath_csv}"
    )
